<template lang="pug">
.ac-ingress-table-wrapper.log-table-wrap
    .table(v-loading="loading")
        el-table(:data="tableData"
            ...
            el-table-column(label="规则类型"
                v-if="columnShows.rule_type"
                prop="rule_type"
                show-overflow-tooltip)

            el-table-column(label="规则名称"
                v-if="columnShows.rule_name"
                prop="rule_name"
                show-overflow-tooltip)

            el-table-column(label="规则描述"
                v-if="columnShows.content"
                prop="content"
                show-overflow-tooltip)

            el-table-column(label="检测结果"
                v-if="columnShows.result"
                prop="result"
                show-overflow-tooltip)

            el-table-column(label="违规操作"
                v-if="columnShows.rule_action"
                prop="rule_action"
                show-overflow-tooltip)
...
export default {
    ...
    methods: {
        ...
        resolveParams (preload = 0) {
            ...
            return logParamsHelper(this.storeParams, rules, 'ac', 'ingress', controlPara);
        },
