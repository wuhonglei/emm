...
.share-query-wrapper.query
    ...
    el-form(v-model="formValid"
        ...
        .query__content
            ...
            .form(v-if="more")
                .form__label {{QUERY_NAME.ruleType}}
                .form__content
                    el-select(v-model="params.ruleType"
                        :loading="loadingStatus.ruleType"
                        @change="changeRuleType"
                        @visible-change="handleRuleTypeVisibleChange")
                        el-option(label="所有" value="")
                        el-option(v-for="item in loadedData.ruleType"
                            :key="item.id"
                            :value="item.name"
                            :label="item.name")
            
            .form(v-if="more")
                .form__label {{QUERY_NAME.ruleAction}}
                .form__content
                    el-select(v-model="params.ruleAction"
                        :loading="loadingStatus.ruleAction"
                        @visible-change="handleActionVisibleChange")
                        el-option(label="所有" value="")
                        el-option(v-for="item in loadedData.ruleAction"
                            :key="item.id"
                            :value="item.name"
                            :label="item.name")
...
export default {
    ...
    methods: {
        ...
        resetList (value) {
            ...
            this.loadedData.ruleType = [];
            this.loadedData.ruleAction = [];
        },
        ...
        handleRuleTypeVisibleChange (v) {
            if (!v || this.hasLoadedData('ruleType')) {
                return;
            }
        ...
        handleActionVisibleChange (v) {
            if (!v || this.hasLoadedData('ruleAction')) {
                return;
            }
        ...
        loadRuleType () {
            this.loadingStatus.ruleType = true;
            loadList(this.normalizedBoxSearchParams(this.params.uuid, 'rule_type'))
                .then((res) => {
                    this.loadedData.ruleType = res.data.data || [];
                })
                .catch(() => {
                })
                .finally(() => {
                    this.loadingStatus.ruleType = false;
                });
        },

        loadAction () {
            this.loadingStatus.ruleAction = true;
            let params = this.normalizedBoxSearchParams(this.params.uuid, 'rule_action');
            params.sort_para.type = this.params.ruleType;

            loadList(params)
                .then((res) => {
                    this.loadedData.ruleAction = res.data.data || [];
                })
                .catch(() => {
                })
                .finally(() => {
                    this.loadingStatus.ruleAction = false;
                });
        },
