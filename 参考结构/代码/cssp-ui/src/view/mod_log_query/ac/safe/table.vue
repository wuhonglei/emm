...
.ac-event-table-wrapper.log-table-wrap
    .table
        el-table(:data="tableData"
            ...
            el-table-column(label="事件类型" v-if="columnShows.attack_type" prop="attack_type" min-width="150px")
            el-table-column(label="用户名" v-if="columnShows.usr_name" prop="usr_name" min-width="150px")
            el-table-column(label="组名" v-if="columnShows.grp_name" prop="grp_name" min-width="150px")
            el-table-column(label="源IP" v-if="columnShows.src_ip" prop="src_ip" min-width="150px")
            el-table-column(label="目的IP" v-if="columnShows.dst_ip" prop="dst_ip" min-width="150px")
            ...
            el-table-column(label="目的端口" v-if="columnShows.serv_port" prop="serv_port" min-width="150px")
            el-table-column(label="终端类型" v-if="columnShows.tm_name" prop="tm_name" min-width="150px")
            el-table-column(label="严重等级" v-if="columnShows.level" prop="level" min-width="150px")
                template(v-slot="{ row }")
                    span(:class="getLevelTextCls(row)") {{row.level}}
            el-table-column(label="访问控制" v-if="columnShows.net_action" prop="net_action" min-width="150px")
            el-table-column(label="url地址" v-if="columnShows.url" prop="url" min-width="150px")
            el-table-column(label="病毒名称" v-if="columnShows.virus" prop="virus" min-width="150px")
                ...
                template(slot-scope="scope")
                    el-popover(placement="left" trigger="click")
                        table-detail(:index="computeIndex(scope.$index)" 
                            :configures="getRowDetailColumns(scope.row)"
...
import { COLUMNS, DETAIL_COLUMNS } from './consts.js';
...
export default {
    ...
    methods: {
        ...
        getRowDetailColumns (row) {
            let res = [].slice.call(COLUMNS);
            let type = row.identify;
        ...
        resolveParams (preload = 0) {
            ...
            return logParamsHelper(this.storeParams, rules, 'ac', 'netsafe', controlPara);
        },
