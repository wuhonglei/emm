...
.web-query-wrapper.query
    ...
    el-form(v-model="formValid"
        ...
        .query__content
            ...
            .form 
                .form__label {{QUERY_NAME.mailType}}
                .form__content
                    el-select(v-model="params.mailType")
                        el-option(label="所有" value="")
                        el-option(label="发送邮件" value="发送邮件")
                        el-option(label="接收邮件" value="接收邮件")
            .form(v-if="more") 
                .form__label {{QUERY_NAME.emailFrom}}
                .form__content
                    el-form-item(prop="emailFrom")
                        el-input(v-model="params.emailFrom"
                            size="small"
                            placeholder="请输入发件地址")
            .form(v-if="more") 
                .form__label {{QUERY_NAME.emailTo}}
                .form__content
                    el-form-item(prop="emailTo")
                        el-input(v-model="params.emailTo"
                            size="small"
                            placeholder="请输入收件地址")
            .form 
...
export default {
    ...
    methods: {
        ...
        handleSiteNameVisibleChange (v) {
            if (!v || Array.isArray(this.loadedData.siteName) &&
                this.loadedData.siteName.length > 0) {
                return;
            }
        ...
        handleTmNameVisibleChange (v) {
            if (!v || Array.isArray(this.loadedData.tmName) &&
                this.loadedData.tmName.length > 0) {
                return;
            }
