...
.ac-behavior-post-wapper.query
    ...
    el-form(v-model="formValid"
        ...
        .query__content
            ...
            .form
                .form__label {{QUERY_NAME.communicateTool}}
                .form__content
                    el-select-tree(v-model="params.communicateTool"
                        :loading="loadingStatus.communicateTool"
                        @visible-change="handleAppNameVisibleChange"
                        :tree-data="loadedData.communicateTool"
                        :multiple="true"
                        value-type="node"
                        tree-label="name"
                        tree-node-key="name"
...
export default {
    ...
    watch:{
        more (show) {
            if (show) {
                let select = ['tmName', 'siteName', 'communicateTool'];
    ...
    methods: {
        ...
        handleAppNameVisibleChange (v) {
            if (!v || this.hasLoadedData('communicateTool')) {
                return;
            }
        ...
        loadAppName () {
            this.loadingStatus.communicateTool = true;
            loadList(this.normalizedBoxSearchParams(this.params.uuid, 'tm_tool'))
                .then((res) => {
                    this.loadedData.communicateTool = res.data.data || [];
                    this.setDefaultAll(this.loadedData.communicateTool, this.params, 'communicateTool');
                    
                })
                .catch(() => {
                })
                .finally(() => {
                    this.loadingStatus.communicateTool = false;
                });
        },
