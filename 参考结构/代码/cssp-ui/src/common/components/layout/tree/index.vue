<template>
    <div class="left-tree-wrapper"
        ...
        <div class="tree__title">
            ...
            <i v-if="open"
               class="title__icon iconfont icon-menu-collapse"
               @click="toggle"></i>
            <i v-if="!open"
               class="title__icon iconfont icon-menu-expand"
               @click="toggle"></i>
        </div>
...
export default {
    ...
    methods: {
        ...
        async loadTreeData () {
            ...
            this.treeData = await this.filterTreeDataByPerm(navs);
        },
        ...
        async filterTreeDataByPerm (routes) {
            let res = [];
            for (let nav of routes) {
                if ((await checker.check({ to: nav, travers: true })).success) {
                    let _nav = Object.assign({}, nav);
                    _nav.children = null;
                    res.push(_nav);
                    if (nav.children) {
                        _nav.children = await this.filterTreeDataByPerm(nav.children);
                    }
                }
            }
            return res;
